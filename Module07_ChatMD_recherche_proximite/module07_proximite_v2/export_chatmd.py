...
def patch_module07_search(content: str) -> str:
    """Remplace la recherche BAN fictive par le moteur statique publié."""
    replacement = f"""## SCR_PASS_SEARCH_MENU

### Trouver les centres les plus proches

La recherche fonctionne avec un **code postal** ou le **nom d’une commune**.
Elle calcule automatiquement les trois centres FRATE les plus proches et affiche
leurs prochaines sessions.

1. [Ouvrir la recherche de proximité]({MODULE07_SEARCH_URL})
2. [Choisir un centre par région](SCR_PASS_REGIONS)
3. [Retour au module](SCR_PASS_MENU)

## SCR_PASS_INPUT_COMMUNE

### Rechercher depuis ma commune

1. [Saisir ma commune ou mon code postal]({MODULE07_SEARCH_URL})
2. [Choisir un centre par région](SCR_PASS_REGIONS)
3. [Retour au module](SCR_PASS_MENU)

## SCR_PASS_INPUT_CP

### Rechercher avec mon code postal

1. [Saisir mon code postal]({MODULE07_SEARCH_URL})
2. [Choisir un centre par région](SCR_PASS_REGIONS)
3. [Retour au module](SCR_PASS_MENU)

## SCR_PASS_INPUT_ADDRESS

### Rechercher un centre proche

Pour protéger vos données, indiquez uniquement votre commune ou votre code
postal : votre adresse complète n’est pas nécessaire.

1. [Ouvrir la recherche]({MODULE07_SEARCH_URL})
...
1. [Ouvrir la recherche]({MODULE07_SEARCH_URL})
2. [Voir les centres par région](SCR_PASS_REGIONS)
3. [Retour au module](SCR_PASS_MENU)

## SCR_PASS_INPUT_DEPT

### Rechercher depuis mon département

Saisissez le code postal de votre commune pour obtenir un classement précis.
...
### Calculer les centres les plus proches
...
"""
    pattern = re.compile(
        r"## SCR_PASS_SEARCH_MENU\s.*?(?=## SCR_PASS_REGION_AUVERGNE)",
        flags=re.DOTALL,
    )
...
class ExportApplication:
    ...
    def _write_module_files(
        ...
        for module_name, module_screens in sorted(grouped.items()):
            ...
            rendered = renderer.render_module(module_name, module_screens)
            if Path(filename).name == "07_passer_examen.md":
                rendered = patch_module07_search(rendered)
